...
func IsPangram(in string) bool {
	var codes []int = make([]int, 'Z'+1) // create an int slice with indecies from 0 to 90
	for i := 'A'; i <= 'Z'; i++ {        //uppercase letters from A to Z have ascii codes from 65 to 90
	...
	for _, letter := range in {
		...
		if uppercase >= 'A' && uppercase <= 'Z' { //runes are just codes, in case of ascii -- ascii codes, here is a chek that it's a letter
